...
func SendOrderFragmentToTarget(target identity.MultiAddress, to identity.Address, from identity.MultiAddress, orderFragment *compute.OrderFragment, timeout time.Duration) error {
	...
	return errors.New(fmt.Sprintf("sendOrderFragment error : %s\nfrom [%s] to [%s]", err.Error(),from.String(),target.String() ))
}
...
func SendOrderFragmentCommitmentToTarget(target identity.MultiAddress, from identity.MultiAddress, timeout time.Duration) error {
	...
	return errors.New(fmt.Sprintf("sendOrderFragmentCommitment error : %s\nfrom [%s] to [%s]", err.Error(),from.String(),target.String() ))
}
...
//func GetFinalsFromTarget(target identity.MultiAddress, traderAddress identity.MultiAddress, timeout time.Duration) ([]*compute.Shard, error) {
//	results := make([]*compute.Shard, 0)
//	conn, err := Dial(target, timeout)
//	if err != nil {
//		return nil, err
//	}
