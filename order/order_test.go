...
var _ = Describe("Orders and order fragments", func() {
	...
	Context("when serializing IDs to strings", func() {
		...
		It("should return the string representation of the ID", func() {
			order := .NewOrder(.OrderTypeLimit, .OrderParityBuy, time.Now().Add(time.Hour), .CurrencyCodeBTC, .CurrencyCodeETH, big.NewInt(10), big.NewInt(1000), big.NewInt(100), big.NewInt(0))
	...
	Context("when testing for equality", func() {
		...
		It("should return false for orders that are not equal", func() {
			lhs := .NewOrder(.OrderTypeLimit, .OrderParityBuy, time.Now().Add(time.Hour), .CurrencyCodeBTC, .CurrencyCodeETH, big.NewInt(10), big.NewInt(1000), big.NewInt(100), big.NewInt(0))
			rhs := .NewOrder(.OrderTypeLimit, .OrderParityBuy, time.Now().Add(time.Hour), .CurrencyCodeBTC, .CurrencyCodeETH, big.NewInt(10), big.NewInt(1000), big.NewInt(100), big.NewInt(1))
	...
	Context("when subtracting order fragments", func() {
		...
		It("should always subtract the sell from the buy", func() {
			lhs := .NewOrder(.OrderTypeLimit, .OrderParityBuy, time.Now().Add(time.Hour), .CurrencyCodeBTC, .CurrencyCodeETH, big.NewInt(10), big.NewInt(1000), big.NewInt(100), big.NewInt(0))
			rhs := .NewOrder(.OrderTypeLimit, .OrderParitySell, time.Now().Add(time.Hour), .CurrencyCodeBTC, .CurrencyCodeETH, big.NewInt(10), big.NewInt(1000), big.NewInt(100), big.NewInt(1))
			Ω(lhs.Equals(rhs)).Should(Equal(false))
