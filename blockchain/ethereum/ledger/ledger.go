...
func (ledger *RenLedgerContract) OpenBuyOrder(signature [65]byte, id order.ID) error {
	...
	tx, err := ledger.binding.OpenBuyOrder(ledger.transactOpts, signature[:], id, common.FromHex("0x0000000000000000000000000000000000000000"))
	if err != nil {
		return err
	}
...
func (ledger *RenLedgerContract) OpenSellOrder(signature [65]byte, id order.ID) error {
	...
	tx, err := ledger.binding.OpenSellOrder(ledger.transactOpts, signature[:], id, common.FromHex("0x0000000000000000000000000000000000000000"))
	if err != nil {
		return err
	}
