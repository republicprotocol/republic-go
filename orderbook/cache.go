...
func (cache *Cache) Open(ord order.Order) error {
	cache.ordersMu.Lock()
	defer cache.ordersMu.Unlock()

	// Check if the order has already been opened
	if entry, ok := cache.orders[string(ord.ID)]; ok && entry.Status != order.Open {
...
func (cache *Cache) Match(ord order.Order) error {
	cache.ordersMu.Lock()
	defer cache.ordersMu.Unlock()
...
func (cache *Cache) Confirm(ord order.Order) error {
	cache.ordersMu.Lock()
	defer cache.ordersMu.Unlock()
	...
	if _, ok := cache.cancels[string(ord.ID)]; ok {
		delete(cache.cancels, string(ord.ID))
	}
...
// is valid and it's status is 'unconfirmed'.
func (cache *Cache) Release(ord order.Order) error {
	cache.ordersMu.Lock()
...
func (cache *Cache) Settle(ord order.Order) error {
	cache.ordersMu.Lock()
	defer cache.ordersMu.Unlock()
...
func (cache *Cache) Cancel(ord order.Order) error {
	cache.ordersMu.RLock()
	cache.cancelMu.Lock()
	defer cache.ordersMu.RUnlock()
	defer cache.cancelMu.Unlock()

	msg, ok := cache.orders[string(ord.ID)]
	if !ok {
	...
	switch msg.Status {
	case order.Open:
		entry := cache.orders[string(ord.ID)]
		entry.Status = order.Canceled
		cache.storeOrderMessage(entry)
	case order.Unconfirmed:
		cache.cancels[string(ord.ID)] = struct{}{}
	default:
	...
	cache.cancels[string(ord.ID)] = struct{}{}
	delete(cache.orders, string(ord.ID))
