...
func (pool *ClientPool) findOrCreateClientConnection(to identity.Multiaddress) (*Client, error) {
	...
	if len(pool.cache) >= pool.cacheLimit {
		var k string
		var kClientCacheEntry *ClientCacheEntry
		for multiaddress := range pool.cache {
			if kClientCacheEntry == nil || kClientCacheEntry.Timestamp.After(pool.cache[multiaddress]) {
				k = multiaddress
				kClientCacheEntry = pool.cache[multiaddress]
			}
		}
		delete(pool.cache, k)
	}
