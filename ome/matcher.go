...
func (matcher *matcher) resolveValues(values []uint64, networkID smpc.NetworkID, com Computation, callback MatchCallback, stage ResolveStage) {
	...
	if !matcher.checkFragmentStatus(com) {
		logger.Compute(logger.LevelDebug, fmt.Sprintf("stopped resolving buy=%v, sell=%v: at least one of the orders has been confirmed", com.Buy.OrderID, com.Sell.OrderID))
		return
	}
...
func buildJoin(com Computation, stage ResolveStage) (smpc.Join, smpc.JoinCommitments, error) {
	...
	if com.Buy.Blinding.Int != nil && com.Sell.Blinding.Int != nil {
		blinding.Int = big.NewInt(0).Add(com.Buy.Blinding.Int, big.NewInt(0).Sub(shamir.CommitP, com.Sell.Blinding.Int))
