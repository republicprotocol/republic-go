...
func main() {
	...
	store, err := leveldb.NewStore(*dataParam, time.Hour)
	if err != nil {
	...
	if len(conn.Config.Tokens) == 0 {
		tokens = contract.TokenAddresses(conn.Config.Network)
	} else {
		tokens = conn.Config.Tokens
	}
	statusProvider.WriteTokens(tokens)
