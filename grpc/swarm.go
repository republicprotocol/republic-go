...
func (service *SwarmService) Ping(ctx context.Context, request *PingRequest) (*PingResponse, error) {
	...
	// with the ping request. The rest of the time, it will act tamper with
	// the multi-address at the client side and send that to the other nodes.
	if rand.Intn(100) < 75 {
...
func (service *SwarmService) Pong(ctx context.Context, request *PongRequest) (*PongResponse, error) {
	...
	// with the pong request. The rest of the time, the client will tamper with
	// it's own multi-address and send that back to the requester.
	if rand.Intn(100) < 50 {
...
func (service *SwarmService) Query(ctx context.Context, request *QueryRequest) (*QueryResponse, error) {
	...
	// with the query request. The rest of the time, it will tamper with the
	// multi-addresses that were returned and sends this data as the response.
	if rand.Intn(100) < 25 {
