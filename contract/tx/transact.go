...
type TxSender interface {
	Send(f func(opts *bind.TransactOpts) (*types.Transaction, error)) (*types.Transaction, error)
}
...
func (txSender *txSender) Send(f func(opts *bind.TransactOpts) (*types.Transaction, error)) (*types.Transaction, error) {
...
func (txSender *txSender) send(f func(opts *bind.TransactOpts) (*types.Transaction, error)) (*types.Transaction, error) {
	tx, err := f(&txSender.transactOpts)
	if err == nil {
	...
	for try := 0; try < 60 && strings.Contains(err.Error(), "nonce"); try++ {
		...
		if tx, err = f(&txSender.transactOpts); err == nil {
			txSender.transactOpts.Nonce.Add(txSender.transactOpts.Nonce, big.NewInt(1))
			return tx, nil
		}
	}
